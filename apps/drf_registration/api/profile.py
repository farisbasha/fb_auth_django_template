...
from apps.drf_registration.api.user_list import CustomUserSerializer
from apps.drf_registration.models import CustomUser

from apps.drf_registration.settings import drfr_settings
from apps.drf_registration.utils.common import import_string, import_string_list
from apps.drf_registration.utils.users import get_all_users, get_user_serializer
...
class OtherUserProfileUpdateView(RetrieveUpdateAPIView):
    """
    Get update user profile information
    """
    ...
    queryset = CustomUser.objects.filter(is_superuser=False ).exclude(role=CustomUser.USER_ROLES.ADMIN)
